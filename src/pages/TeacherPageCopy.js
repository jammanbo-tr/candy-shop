...
const TeacherPageCopy = () => {
  ...
  const [studentsSnapshot, studentsLoading, studentsError] = useCollection(collection(db, 'copy_students'));
  const [notificationsSnapshot, notificationsLoading, notificationsError] = useCollection(collection(db, 'copy_notifications'));
  ...
  const handleUpdateNotification = async (notificationId, updates) => {
    try {
      const notificationRef = doc(db, 'copy_notifications', notificationId);
    ...
    return (
      <div style={{position:'fixed',top:0,left:0,width:'100vw',height:'100vh',background:'rgba(0,0,0,0.25)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:99999}}>
        <div style={{background:'#fff',padding:36,borderRadius:18,minWidth:320,boxShadow:'0 4px 32px #b2ebf240',textAlign:'center'}}>
          <h2 style={{color:'#1976d2',marginBottom:18}}>교사용 비밀번호 입력 (사본)</h2>
  ...
  const handleOpenCouponBox = async (student) => {
    ...
    const studentRef = doc(db, 'copy_students', student.id);
