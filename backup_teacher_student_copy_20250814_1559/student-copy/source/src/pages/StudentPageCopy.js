...
import EmotionAttendanceModal from '../components/EmotionAttendanceModalCopy';
...
const StudentPage = () => {
  ...
  const [studentDoc, loading, error] = useDocument(doc(db, 'copy_students', studentId));
  const [studentsSnapshot] = useCollection(collection(db, 'copy_students'));
  const [itemsSnapshot] = useCollection(collection(db, 'copy_items'));
  ...
  useEffect(() => {
    if (student && student.money !== undefined && student.balance === undefined) {
      ...
      updateDoc(doc(db, 'copy_students', studentId), {
    ...
    const checkTodayEmotionSubmission = async () => {
      ...
      console.log('🌈 감정출석부 상태 체크 시작', {
        ...
        checkingDate: today
          ...
          // 1. 새로운 서브컬렉션에서 먼저 확인
          const emotionRef = doc(db, 'copy_students', student.id, 'emotions', today);
          const newEmotionSnapshot = await getDoc(emotionRef);
          let hasSubmitted = newEmotionSnapshot.exists();
          let dataSource = 'new';
          let emotionData = null;

          // 2. 새 구조에 없으면 기존 컬렉션에서 확인 (호환성)
          if (!hasSubmitted) {
            ...
            hasSubmitted = !legacySnapshot.empty;
            dataSource = 'legacy';
            
            if (hasSubmitted) {
              emotionData = legacySnapshot.docs[0].data();
            }
          } else {
            emotionData = newEmotionSnapshot.data();
          }
          ...
          console.log('🔍 감정출석 확인 결과:', {
            ...
            legacyStructureChecked: !newEmotionSnapshot.exists()
          });

          // 실제 문서 데이터도 확인
          if (hasSubmitted && emotionData) {
            console.log('📄 오늘의 감정출석 데이터:', {
              source: dataSource,
              date: emotionData.date,
              emotion: emotionData.emotion,
              intensity: emotionData.intensity,
              cause: emotionData.cause,
              timestamp: emotionData.timestamp?.toDate?.()?.toLocaleString('ko-KR')
            });
          }

          // 최근 감정출석 기록들도 확인 (새 구조 + 기존 구조)
          const emotionsCollectionRef = collection(db, 'copy_students', student.id, 'emotions');
          const newRecentQuery = query(emotionsCollectionRef, orderBy('date', 'desc'), limit(5));
          const newRecentSnapshot = await getDocs(newRecentQuery);
          
          const legacyRecentQuery = query(
            collection(db, 'emotionAttendance'),
            where('studentId', '==', student.id),
            orderBy('date', 'desc'),
            limit(5)
          );
          const legacyRecentSnapshot = await getDocs(legacyRecentQuery);
          
          const newRecords = newRecentSnapshot.docs.map(doc => ({
            id: doc.id,
            date: doc.data().date,
            emotion: doc.data().emotion,
            timestamp: doc.data().timestamp?.toDate?.()?.toLocaleString('ko-KR'),
            source: 'new'
          }));
          
          const legacyRecords = legacyRecentSnapshot.docs.map(doc => ({
            id: doc.id,
            date: doc.data().date,
            emotion: doc.data().emotion,
            timestamp: doc.data().timestamp?.toDate?.()?.toLocaleString('ko-KR'),
          ...
          const allRecords = [...newRecords, ...legacyRecords]
            .sort((a, b) => b.date.localeCompare(a.date))
            .slice(0, 5);
          
          console.log('📋 최근 감정출석 기록들 (통합):', allRecords);
          
          setHasSubmittedToday(hasSubmitted);
        
        // 강제로 상태 업데이트를 위해 추가 로그
        if (!hasSubmitted) {
          console.log('✅ 감정출석부 버튼이 활성화되어야 합니다!');
        } else {
          console.log('❌ 오늘은 이미 감정출석을 제출했습니다.');
        }
        
      } catch (error) {
        console.error('❌ 감정출석 확인 오류:', error);
        console.error('❌ 오류 상세:', error.message, error.stack);
        // 오류가 발생하면 기본적으로 미제출 상태로 설정
  ...
  // 퀴즈 알림 수신 (최적화된 폴링 방식)
  useEffect(() => {
    console.log('퀴즈 알림 폴링 시작 (30초 간격)');
    ...
    const checkForNewQuizzes = async () => {
      try {
        ...
        console.log('퀴즈 알림 확인:', snapshot.docs.length);
        ...
        snapshot.docs.forEach(doc => {
          ...
          // 최근 2분 이내에 생성된 알림만 처리 (시간 여유 확대)
          if ((now - notificationTime) < 120000) {
            ...
            if (isTargetedToMe) {
              ...
              if (!seenQuizzes.includes(doc.id)) {
                console.log('새로운 퀴즈 발견 (나에게 해당):', notification);
    ...
    // 30초마다 확인 (3초 → 30초로 변경하여 비용 90% 절약)
    const interval = setInterval(checkForNewQuizzes, 30000);

    return () => {
      console.log('퀴즈 알림 폴링 정리');
    ...
    try {
      ...
      if (isCorrect) {
        ...
        if (Object.keys(updateData).length > 0) {
          await updateDoc(doc(db, 'copy_students', studentId), updateData);
        }
...
출처 : 유튜브 채널 느루`
  ...
  const fetchRecorderMissions = async () => {
    try {
      const recorderDoc = await getDoc(doc(db, 'copy_settings', 'recorderMissions'));
  ...
  const fetchRecorderMissionsAdvanced = async () => {
    try {
      const recorderAdvancedDoc = await getDoc(doc(db, 'copy_settings', 'recorderMissionsAdvanced'));
  ...
  // 새로운 단계 통과 시 자동 축하 모달 표시
  useEffect(() => {
    const currentLevel = getHighestCompletedLevel();
    const storedLevel = localStorage.getItem(`lastCelebrated_${studentId}`);
    
    if (currentLevel !== null && currentLevel > (parseInt(storedLevel) || -1)) {
      // 새로운 단계를 통과했을 때
      setTimeout(() => {
        setShowCelebrationModal(true);
        setCelebrationLevel(currentLevel);
        // localStorage 저장은 사용자가 확인 버튼을 클릭할 때로 이동
      }, 1000); // 1초 지연 후 축하 모달 표시
    }
  }, [recorderMissions, studentId]);

  // 단소급수미션 데이터 실시간 감지
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, 'copy_settings', 'recorderMissions'), (doc) => {
      if (doc.exists()) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, 'copy_settings', 'recorderMissionsAdvanced'), (doc) => {
      if (doc.exists()) {
  ...
  // 새로운 심화 단계 통과 시 자동 축하 모달 표시
  useEffect(() => {
    const currentLevel = getHighestCompletedLevelAdvanced();
    const storedLevel = localStorage.getItem(`lastAdvancedCelebrated_${studentId}`);
    
    if (currentLevel !== null && currentLevel > (parseInt(storedLevel) || 0)) {
      // 새로운 심화 단계를 통과했을 때
      setTimeout(() => {
        setShowAdvancedCelebrationModal(true);
        setAdvancedCelebrationLevel(currentLevel);
        // localStorage 저장은 사용자가 확인 버튼을 클릭할 때로 이동
      }, 1000); // 1초 지연 후 축하 모달 표시
    }
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        const q = query(collection(db, 'copy_notices'), orderBy('updatedAt', 'desc'));
        const snapshot = await getDocs(q);
  ...
  const handleSendPraise = async () => {
    ...
    try {
      await Promise.all(selectedFriends.map(friendId => {
        ...
        return updateDoc(doc(db, 'copy_students', friendId), {
  ...
  const handleAlarmClick = async () => {
    ...
    if (student && (student.messages || []).some(m => m.from === 'teacher')) {
      ...
      await updateDoc(doc(db, 'copy_students', studentId), { lastTeacherMsgRead: lastTs });
    }
    // 모든 알람 읽음 처리 - 개선
    const alarms = ((student?.announce||[]).concat(student?.notifications||[]));
  ...
  useEffect(() => {
    ...
    if (myPraise.length > 0) {
      ...
      updateDoc(doc(db, 'copy_students', studentId), { praise: praiseArr });
      setTimeout(() => setPraiseResultEffect(false), 2000);
    }
    ...
    if (myQuest.length > 0) {
      ...
      updateDoc(doc(db, 'copy_students', studentId), { quests: questArr });
      setTimeout(() => setPraiseResultEffect(false), 2000);
    }
  }, [student]);
  ...
  useEffect(() => {
    if (!studentId) return;
    const unsub = onSnapshot(doc(db, 'copy_students', studentId), (snap) => {
  ...
  const handleUseCoupon = async (coupon) => {
    ...
    await updateDoc(doc(db, 'copy_students', studentId), { coupons: updatedCoupons });
  };
  ...
  useEffect(() => {
    if (!studentLoaded || !student) return;
    // 레벨업으로 증가했을 때만 모달을 띄우도록 조건 강화
    // 이전 레벨을 로컬에 저장하여 실제로 올라갔는지 확인
    const prevLevelKey = `prevLevel_${studentId}`;
    const prevLevel = Number(localStorage.getItem(prevLevelKey) || student.level);
    localStorage.setItem(prevLevelKey, String(student.level));
    const levelIncreased = student.level > prevLevel;
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'copy_settings', 'feverTime'), (snap) => {
      const data = snap.data();
  ...
  const handleSelectEmotion = async (icon) => {
    ...
    await updateDoc(doc(db, 'copy_students', studentId), { emotionIcon: icon });
  };
  ...
  useEffect(() => {
    ...
    // 교사 트리거가 활성화된 경우에만 동작
    if (student.triggerCouponEvent && student.triggerCouponEvent.active === true) {
  ...
  }, [student?.triggerCouponEvent?.active, studentId]);
  ...
  return (
    <div style={{ 
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'stretch', width: '100%', p: 0 }}>
            ...
            <Box sx={{ flex: 1, p: '24px 18px 18px 18px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 180 }}>
              <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', mb: 1 }}>
                ...
                >
                  {selectedEmotion ? (
                    <img src={selectedEmotion} alt="감정" style={{ width: 24, height: 24, objectFit: 'contain' }} />
                  ) : (
                  ...
                  <Box sx={{
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, mt: 3 }}>
                      {EMOTION_ICONS.map((icon, idx) => (
                        ...
                        >
                          <img src={icon} alt={`감정${idx + 1}`} style={{ width: 36, height: 36, objectFit: 'contain' }} />
                        </button>
                      ))}
                    </Box>
                    <Button
                      ...
                      {quest.requestPending ? (
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            await updateDoc(doc(db, 'copy_students', studentId), { quests: newQuests });
                          }}
                        >
                          퀘스트 승인 요청하기
                        </Button>
      ...
      <EmotionAttendanceModal
        ...
        onClose={() => {
          setShowEmotionAttendanceModal(false);
          // 제출 후 상태 업데이트
