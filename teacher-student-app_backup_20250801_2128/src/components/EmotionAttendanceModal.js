...
const EmotionAttendanceModal = ({ isOpen, onClose, student, onSubmitSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!newStructureSnapshot.exists()) {
        ...
        const oldStructureSnapshot = await getDocs(todayQuery);
        hasSubmittedInOldStructure = !oldStructureSnapshot.empty;
      }

      if (newStructureSnapshot.exists() || hasSubmittedInOldStructure) {
        showResult('오늘은 이미 감정출석을 제출했습니다.', 'error');
        setIsSubmitting(false);
        return;
      }

      // 감정출석 데이터 저장 (새로운 구조: students/{id}/emotions/{date})
      const emotionData = {
        date: dateString,
...
export default EmotionAttendanceModal;