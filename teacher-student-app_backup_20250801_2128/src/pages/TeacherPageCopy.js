...
const TeacherPageCopy = () => {
  ...
  const handleAmount = async () => {
    ...
    for (const id of bankSelectedIds) {
      ...
      await updateDoc(doc(db, 'copy_students', id), { balance: newBalance });
    }
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'linear-gradient(135deg, rgba(224,247,250,0.6) 0%, rgba(227,242,253,0.6) 100%), url(/TR_bg.png) center/cover no-repeat' }}>
        <div style={{ background: '#fff', padding: '40px', borderRadius: 28, boxShadow: '0 4px 32px #b2ebf240', textAlign: 'center', maxWidth: 500 }}>
          ...
          >
            <PersonAddIcon style={{ color: '#fff', fontSize: 24 }} />
            <span>68명의 학생 추가하기</span>
          </button>
        </div>
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ minHeight: '100vh', width: '100vw', padding: '32px',
        ...
        {/* 종(알림) 버튼 UI */}
        <div style={{ position: 'fixed', top: 24, right: 32, zIndex: 2000, display: 'flex', flexDirection: 'row', gap: 8, alignItems: 'center' }}>
          {/* 유리병 아이콘 버튼 */}
          <div style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative' }} title="학급 캔디 유리병" onClick={() => setShowJarModal(true)}>
            <img src="/jar2.png" alt="유리병" style={{ width: 28, height: 28, objectFit: 'contain', filter: 'drop-shadow(0 2px 6px #b2ebf2a0)' }} />
          </div>

          {/* 종(알림) 버튼 */}
          <div style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative' }} title="학생 요청 알림" onClick={() => setShowTeacherAlarm(true)}>
            <NotificationsActiveIcon fontSize="medium" color="primary" />
            {pendingRequests.length > 0 && (
              <span style={{ position: 'absolute', top: 0, right: 0, background: '#ff1744', color: '#fff', borderRadius: '50%', width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 'bold', boxShadow: '0 1px 4px rgba(0,0,0,0.15)' }}>
          ...
          {/* 단소급수미션 버튼 */}
          <button onClick={() => {setShowRecorderModal(true); fetchRecorderMissions();}} style={{ background: '#f3e5f5', border: 'none', borderRadius: 999, padding: '6px 12px', boxShadow: '0 2px 8px #b2ebf240', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, color: '#7b1fa2', fontSize: 12 }} title="단소급수미션">
            <MusicNoteIcon style={{ color: '#7b1fa2', fontSize: 20 }} />
            <span style={{ fontWeight: 600, color: '#7b1fa2', fontSize: 12 }}>단소급수</span>
          </button>
          
          {/* 심화 단소급수미션 버튼 */}
          <button onClick={() => {setShowRecorderAdvancedModal(true); fetchRecorderMissionsAdvanced();}} style={{ background: 'linear-gradient(135deg, #667eea, #764ba2)', border: 'none', borderRadius: 999, padding: '6px 12px', boxShadow: '0 4px 16px rgba(102, 126, 234, 0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, color: '#fff', fontSize: 12 }} title="단소급수미션(심화)">
            <MusicNoteIcon style={{ color: '#fff', fontSize: 20 }} />
            <span style={{ fontWeight: 600, color: '#fff', fontSize: 12 }}>심화</span>
          </button>

          {/* 공지사항(사이렌) 버튼 */}
          <button onClick={() => {setShowNoticeModal(true); fetchAlarms();}} style={{ background: '#fffde7', border: 'none', borderRadius: 999, padding: '6px 12px', boxShadow: '0 2px 8px #b2ebf240', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, color: '#f57f17', fontSize: 12 }} title="공지&예약 관리">
            <CampaignIcon style={{ color: '#f57f17', fontSize: 20 }} />
            <span style={{ fontWeight: 600, color: '#f57f17', fontSize: 12 }}>공지</span>
          </button>
          
          <button onClick={()=>setShowBankModal(true)} style={{ background: '#fffde7', border: 'none', borderRadius: 999, padding: '6px 12px', boxShadow: '0 2px 8px #b2ebf240', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, color: '#d72660', fontSize: 12 }}>
            <StorefrontIcon style={{ color: '#d72660', fontSize: 20 }} />
            <span style={{ fontWeight: 600, color: '#d72660', fontSize: 12 }}>캔디숍</span>
          </button>
          
          {/* 새로운 학생 추가 버튼 */}
          <button onClick={addNewStudents} style={{ background: 'linear-gradient(135deg, #4caf50, #8bc34a)', border: 'none', borderRadius: 999, padding: '6px 12px', boxShadow: '0 4px 16px rgba(76, 175, 80, 0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontWeight: 600, color: '#fff', fontSize: 12 }}>
            <PersonAddIcon style={{ color: '#fff', fontSize: 20 }} />
            <span style={{ fontWeight: 600, color: '#fff', fontSize: 12 }}>학생추가</span>
          </button>
        </div>
