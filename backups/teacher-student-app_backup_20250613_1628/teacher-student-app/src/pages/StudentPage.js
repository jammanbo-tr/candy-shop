...
import { doc, updateDoc, arrayUnion, collection, getDocs, query, where, orderBy, onSnapshot, getDoc } from 'firebase/firestore';
...
출처: 피리토끼 선생님`
  ...
  useEffect(() => {
    ...
    if (currentLevel !== null && currentLevel > (parseInt(storedLevel) || -1)) {
      ...
      setTimeout(() => {
        ...
        localStorage.setItem(`lastCelebrated_${studentId}`, currentLevel.toString());
  ...
  return (
    <div style={{ 
      ...
      {/* 우측 하단: 링크 히스토리 & 업데이트 정보 버튼 (과거 스타일 복원) */}
